...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (isChild) {
      ...
      // Type unknown — pre-roll cute/cool for all 9 types server-side, pass lookup table to AI
      const cuteMap = {};
      for (const [t, p] of Object.entries(TYPE_CUTE_PROB)) {
        cuteMap[t] = Math.random() < p ? 'cute' : 'cool';
      }
      const cuteTableStr = Object.entries(cuteMap).map(([t, d]) => t + '=' + d).join(', ');
      const visualTableNote = ' Visual design: pre-rolled style decisions by type: ' + cuteTableStr + '. After choosing the monster\'s type, look up that type in this table and use the exact style shown. cute style = chubby rounded body, large expressive friendly eyes, soft gentle expression, endearing presence; cool style = sleek powerful silhouette, sharp stylish features, confident dynamic expression, legendary cool presence. ALWAYS maintain pokemon-style chibi art (same deformed proportions and line weight) regardless of cute or cool direction. Color palette must strongly reflect the chosen type: 火=red/orange, 水=blue/cyan, 木=green, 雷=yellow/gold, 雲=white/light blue, 土=brown/earthy tan, 氷=white/pale ice blue, 鉄=silver/steel grey, 念=purple/pink (accent colors allowed but base tone must match).';
      // Body build (independent axis)
      const build = rollBuild();
      const buildNote = ' Body build (INDEPENDENT of cute/cool direction):' + BUILD_DESC[build] + '. Combine both axes freely (e.g., heavy+cute = large and endearing; light+cool = small but razor-sharp and stylish).';
      // Move weights based on build
