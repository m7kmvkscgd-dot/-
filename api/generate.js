...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (isChild) {
      ...
      // Type is known — resolve cute/fierce server-side
      ...
      const moveTypeNote = ' Move type combination: pre-rolled value (' + moveTypeRoll + '). If value < 0.80: heavy move = monster\'s own type, light move = a DIFFERENT type from [火,水,木,雷,雲,土,氷,鉄,念] that fits the locale or concept. If value >= 0.80: BOTH moves must be the monster\'s own type.';
      stageNote = 'This is the ADULT form. Make it look mature and fully evolved. The monster must have EXACTLY 1 type (types array must have exactly one element). Write the lore in Japanese in 2-3 short sentences only.' + visualTableNote + movesNote + moveTypeNote;
    }
